  ...
  async execute(member, client) {
    try {
      ...
      const embWelcome = new MessageEmbed()
        ...
        .setTitle("Welcome 👋")
        .setDescription(`Chào mừng ${member.user} tham gia server **${member.guild.name}!**  😍`)
        .addField(`Bạn là thành viên thứ ${member.guild.memberCount} của server`, "Chúc bạn một ngày làm việc vui vẻ!")
        .setColor("#00BCE3")
      ...
      await member.guild.channels.cache.get(welcomeID).send({ embeds: [embWelcome] });
      ...
      const emLog = new MessageEmbed()
        ...
        .addFields(
          ...
          { name: "Created at:", value: `${moment(member.createdAt).format("dddd, MMMM Do YYYY, HH:mm")}` }
        );
